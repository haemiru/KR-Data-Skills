...
2. **인증키** — `credentials` 스킬의 프로토콜을 따른다. 필요한 변수는
   ...
   > 거래 유형별 신청 URL은 `references/endpoints.md`의 데이터셋 표에 있다.
   > 전부 자동승인이라 신청 즉시 쓸 수 있다. `check-key`로 확인한다.
...
- **호출 예산** — 개발계정 **일 10,000건**(실거래가 계열. 나라장터 1,000건과 다르다).
  이 API는 **월 하나당 최소 1회 호출**이다.
...
`cdealType == "O"` 가 해제된 거래다. **유형마다 비율이 크게 다르다**(실측):

| `--type` | 해제 비율 | 표본 |
|---|---|---|
| **`silv-trade`** 분양권 전매 | **10.81%** | 111건 |
| `apt-trade` 아파트 매매 | 2.35% | 3,443건 |
| `rh-trade` 연립다세대 | 2.50% | 40건 (표본 작음) |
| 전체 | 2.58% | 3,683건 |

**분양권 전매가 일반 매매의 4배가 넘는다.** 계약금만 걸고 전매했다가
해제하는 경우가 많은 것으로 보인다. 분양권 시세를 낼 때는 해제를 안 빼면
심하게 왜곡된다.

**시세·평균가·최고가를 낼 때 반드시 빼야 한다.** 최고가는 한 건만 섞여도
답이 바뀐다.
...
## 🔴 최근 1~2개월은 데이터가 덜 찼다 — 거래량 결론 금지

부동산 거래신고 기한이 **계약 후 30일**이라, 최근 월은 아직 신고가 안 들어와 있다.
**강남구 실측 (2026-08-05 조회):**
...
**시장이 식은 게 아니라 신고가 안 들어온 것이다.** 이걸 모르면
"7월 거래량이 반토막 났다"는 **완전한 오답**을 만든다.

- 조회 범위에 당월·직전월이 있으면 래퍼가 **🔴 경고를 출력**한다
- 요청한 월 중 일부만 0건이어도 **⚠ 경고**한다 (전체가 0건이 아니면 조용히 지나가던 구멍)
- **추이·거래량을 볼 거면 최근 2개월을 빼거나, 미완성임을 함께 보고한다**
- 시세(평균가)도 표본이 작아져 흔들린다. 최근 월만으로 시세를 단정하지 말 것

## 단지명으로 찾을 때 — API에 이름 필터가 없다

**"○○아파트 얼마에 팔렸어"는 API가 직접 지원하지 않는다.**
지역·기간으로 받아서 **로컬에서 걸러야** 한다.

```bash
# 1) 지역·기간으로 받는다 (단지명 필드를 반드시 포함)
uv run $S search --type apt-trade --region 11680 --from 202604 --to 202606 \
  --fields "aptNm,dealAmount,excluUseAr,floor,umdNm" --output out/gangnam.json

# 2) 로컬에서 단지명으로 거르고 집계한다
uv run python -X utf8 -c "
import json,sys,statistics; sys.stdout.reconfigure(encoding='utf-8')
rows=json.load(open('out/gangnam.json',encoding='utf-8'))['items']
hits=[r for r in rows if '까치마을' in r['aptNm'] and not r['_cancelled']]
ps=[r['_dealAmountWon'] for r in hits]
print(len(hits),'건  중앙', statistics.median(ps)/1e8, '억')
"
```

- **부분일치로 거른다.** `aptNm`이 `까치마을2단지`처럼 붙어 있는 경우가 많다
- **해제 거래(`_cancelled`)를 반드시 뺀다.** 최고가는 한 건만 섞여도 답이 바뀐다
- 같은 단지도 **면적·층에 따라 가격이 크게 다르다.** 하나로 뭉뚱그리지 말 것
- 못 찾으면 **범위를 넓히기 전에 지역코드부터 의심한다**(단지가 다른 구에 있을 수 있다)
...
| **최근 달 거래량이 급감** | 🔴 **시장이 아니라 신고 지연**(계약 후 30일). 실측 459→222→112→0 | **거래량 결론 금지.** 최근 2개월을 빼거나 미완성임을 밝힐 것 |
| 여러 달 중 한 달만 0건 | 전체가 0건이 아니라 예전엔 조용히 지나갔다 | 래퍼가 이제 ⚠ 경고한다. 그 달을 "거래 없음"으로 보고하지 말 것 |
| 단지명으로 못 찾겠다 | **API에 이름 필터가 없다** | 지역·기간으로 받아 `aptNm` 부분일치로 로컬 필터 (위 절 참조) |
