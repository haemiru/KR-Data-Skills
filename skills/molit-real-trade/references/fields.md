...
`RTMSDataSvcSilvTrade`. 서울 종로구엔 0건이라 평택시(`41220`)로 확인했다.

공통 11개 + 아래.

| 필드 | 뜻 | 관측값 |
|---|---|---|
| `aptNm` | 아파트명 | `평택 브레인시티 2BL 대광로제비앙 모아엘가` |
| `excluUseAr` | 전용면적(㎡) | `80.9903` |
| `floor` | 층 | `5` |
| `sggNm` | 시군구명 | `평택시` |
| `buyerGbn` / `slerGbn` | 매수자 / 매도자 구분 | `개인` `법인` |
| `ownershipGbn` | 소유권 구분 ※**표본 111건 전부 공란** | — |

**아파트 매매(32개)에 있는데 분양권에 없는 것 16개** — `buildYear`(미완공이라
당연) · `aptSeq` · `aptDong` · `umdCd` · `bonbun`/`bubun` · `landCd` ·
`landLeaseholdGbn` · `rgstDate` · 도로명 계열 7개 전부.

> 지번 마스킹 형태가 또 다르다 — `147` · `가-` · `산184-2` 가 섞여 나온다.
> 미준공 단지라 블록 표기(`가-`)가 그대로 오는 것으로 보인다.

## 12. 🔴 해제된 거래 — `cdealType` ✅ 실측
...
|---|---|---|
| **분양권 전매** | 12 / 111 | **10.81%** |
| 연립다세대 매매 | 1 / 40 | 2.50% (표본 작음) |
| 아파트 매매 | 81 / 3,443 | 2.35% |
| 토지·상업·오피스텔·단독 | 0 / 85 | 0% (표본 작음) |
| **전체** | 95 / 3,683 | **2.58%** |

> **분양권 전매의 해제율이 일반 매매의 4배가 넘는다.** 계약금만 걸고
> 전매했다가 해제하는 경우가 많은 것으로 보인다. 분양권 시세 분석에서는
> 해제 제외가 특히 중요하다.
>
> 공장창고는 4건 중 1건(25%)이 해제였지만 **표본이 4건이라 의미 없다.**
...
## 12-2. 🔴 신고 지연 — 최근 월은 데이터가 덜 찼다 ✅ 실측

부동산 거래신고 기한이 **계약 후 30일**이라, 최근 월은 신고가 진행 중이다.

**서울 강남구 아파트 매매, 2026-08-05 조회 시점의 월별 신고 건수:**
...
**단조 감소가 시장 신호가 아니다.** 같은 지역을 한 달 뒤에 다시 조회하면
202607·202608 수치가 올라간다.

- 이 표는 **한 시점의 스냅샷**이다. 조회일이 바뀌면 최근 월 숫자가 달라진다
- 202602~202605의 오르내림은 계절성·시장 요인일 수 있다 — **신고 지연만으로
  설명하지 말 것.** 확실한 건 **당월·직전월이 미완성**이라는 것뿐이다
- 래퍼가 조회 범위에 당월·직전월이 있으면 🔴 경고하고,
  요청 월 중 일부만 0건이어도 ⚠ 경고한다

> **거래량 비교·추이 분석에서 최근 2개월을 그대로 쓰면 오답이 나온다.**
> 빼거나, 미완성임을 결과에 함께 밝힌다.

## 12-3. 단지명 검색은 API가 지원하지 않는다

요청 파라미터에 건물명이 없다. `aptNm`은 **응답 전용**이다.
(나라장터의 `prtcptLmtRgnNm`이 요청 전용이었던 것과 정반대 방향의 함정이다.)

지역·기간으로 받아 **로컬에서 부분일치로 거른다.** 절차는 SKILL.md 참조.
...
| 항목 | 상태 |
|---|---|
| `apt-trade-old`(구버전) 필드 차이 | ❌ 경로 생존만 확인. 기본이 `apt-trade`라 실사용엔 영향 없음 |
| `ownershipGbn`(분양권) 값 목록 | ❌ 표본 111건 전부 공란 |
| `buildingUse` · `jimok` · `landUse` 값 목록 | 🟡 일부만 관측 |
| `useRRRight`(갱신요구권) 값 목록 | 🟡 표본에서 대부분 공란 |
| 유형별 해제율 (분양권·아파트 외) | 🟡 표본 40건 이하라 신뢰 구간이 넓다 |
| 채움률 전반 | 🟡 표본이 좁아 일반화 불가 |

> **13개 유형 중 12개의 필드가 실측으로 확정됐다.** 남은 `apt-trade-old` 는
> 기본값(`apt-trade`)이 상위 호환이라 실사용에 걸리지 않는다.
