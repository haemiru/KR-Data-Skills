...
3. 마이페이지 → 오픈API → 개발계정 상세에서 **일반 인증키**를 복사한다.
   ...
   래퍼가 자동 판별한다.
4. 사용자에게 아래 명령을 **직접 터미널에서** 실행하도록 안내한다.
...
전부 `skills/g2b_bid_notice/` 에서 실행한다.
...
| 결과 0건 | `inqryDiv` 의미가 업무구분마다 다름 | `--inqry-div` 를 바꿔 보고, 기간을 넓혀 볼 것 |
...
- 응답 필드: `references/fields.md` — **일부 미검증이다. 그 문서의 경고를 읽을 것.**
